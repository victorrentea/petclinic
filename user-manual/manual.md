...
The list shows registered owners one page at a time. Each row includes the address, city, telephone, and the names of the pets associated with that owner. Above the table is a *Last name* search box — type a last name (or any prefix) and click *Find Owner* to narrow the list to matching owners.

Click the *Name* or *City* column heading to sort the list by that column; click the same heading again to reverse the direction. The arrow beside the heading shows how the list is currently sorted (▲ ascending, ▼ descending), and a faint ↕ marks the other column you can sort by. Click an owner's name to open their full record.

Below the table is a paginator for stepping through the results. Use the *Owners per page* selector to show 5, 10, or 20 owners at a time, read off how many owners matched ("Showing 1–10 of 28"), and use the arrows to jump to the first, previous, next, or last page. The *Add Owner* button below the list opens the new-owner form.
