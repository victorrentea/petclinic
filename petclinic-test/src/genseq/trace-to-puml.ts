...
/**
 * The section header's own handle: `genseq-scenario://<line>`, resolved by whatever renders
 * the SVG against the test file the diagram is already filed next to.
 *
 * A line number, not a path. The .puml is committed and read on other machines, so it cannot
 * carry an absolute path — and it does not need to: the file is derivable from the diagram's
 * own name, which is the same reasoning that lets the review page label it `generated by …`.
 */
const SCENARIO_SCHEME = 'genseq-scenario://';
...
export interface DiagramScenario {
  ...
  steps?: StepMark[];
  /**
   * Which line of the test file this scenario is written on, so its section header can be
   * a link back to it. Filled in at render time (see renderScenarios) and deliberately not
   * cached with the spans: a re-render must point at where the scenario is now.
   */
  line?: number;
}
...
export function renderDiagram(
  ...
  for (const scenario of scenarios) {
    ...
    sections.push({title: scenario.title, lines, line: scenario.line});
  }
  ...
  const body = sections.flatMap((s) => [`== ${sectionTitle(s)} ==`, ...s.lines]);
  return {
...
/** The header text, as a link to the scenario when its line is known, else as plain text. */
function sectionTitle(section: DiagramSection): string {
  if (section.line === undefined) return section.title;
  return `[[${SCENARIO_SCHEME}${section.line}{Open the test at this scenario} ${section.title}]]`;
}
...
interface DiagramSection {
  title: string;
  lines: string[];
  /** 1-based line of the scenario in the test file, when it could be found. */
  line?: number;
}
