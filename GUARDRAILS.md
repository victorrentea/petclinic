...
| `ConceptModelDiagramTest` | Concept map drift | Reads the mxGraph XML embedded in `petclinic-backend/docs/ConceptModel.drawio.png` — the hand-arranged map of the domain, kept alongside the generated `DomainModel.puml` because a regenerated diagram cannot stay in the same place, and people navigate this one from memory. Boxes carry `concept="Owner"`, lines `assoc="Owner-Pet"`; cardinalities are checked against the **visible** end labels — a bare `*` at a many end, nothing at a to-one end — so the picture cannot pass while showing a marker it does not mean, and an unmarked end is asserted to really be to-one. Fails both ways on concepts and on links, rejects a line between two concepts that declares no `assoc`, and fails while any concept still sits untouched in the staging lane `docs/scripts/concept-model-patch.py` parked it in — layout is the one thing here a human owns |
