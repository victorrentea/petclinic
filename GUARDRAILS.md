...
| `EndpointComplexityExtractorTest` | Where complexity actually sits | ASM reads `target/classes`: McCabe CC per method + the call graph from every entry point (`@RestController` handlers, `@McpTool`s, `@Scheduled` jobs, message listeners) down (MapStruct impls and lambdas included, abstract calls re-pointed at their implementations). Renders `docs/generated/endpoint-complexity.{html,json}`, endpoints ranked by the decision points of the *whole flow*. A report, not yet a fail-gate — no threshold is enforced |
