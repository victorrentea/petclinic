...
| `DeploymentDiagramTest` | Deployment diagram drift | Reads the mxGraph XML embedded in `petclinic-backend/docs/deployment.drawio.png` (a real draw.io file: one artifact that both renders on GitHub and opens for editing in the desktop app) and compares its arrows against `petclinic-test/src/*.genseq.puml`, generated from real OpenTelemetry traces. Containers carry `traceParticipant="Browser|Backend|DB"`, arrows carry `traced="yes|no"`. Fails both ways — a traced arrow no trace shows, and a call the traces show that the picture does not draw — plus a third check that no arrow between two traced containers is left undeclared, so a new connection cannot opt out of the guardrail in silence |
...
| `DomainModelExtractorTest` | Domain class diagram | Regenerates `docs/generated/DomainModel.puml` from the domain classes by plain reflection (no JPA annotations read). Notation: a **single arrow** where only one class declares a field for the other, a **double arrow** where both do, so navigability is visible instead of hidden behind an undirected line; the **role name sits in the end label of the class it names**, next to that end's multiplicity, rather than dangling off the line; multiplicity is trimmed to what informs — `*` for many, **nothing** for one. The `*` is written `~*` in the source because PlantUML runs end labels through Creole, where a leading `* ` is a bullet |
| `EndpointComplexityExtractorTest` | Where complexity actually sits | **Sonar Cognitive Complexity** of the *whole flow* behind every entry point (`@RestController` handlers, `@McpTool`s, `@Scheduled` jobs, message listeners). Two readers, each asked what it can answer: ASM over `target/classes` builds the call graph (lambdas, method references, MapStruct impls, abstract calls re-pointed at their implementations); JavaParser over `src/main/java` + `target/generated-sources/annotations` scores each method, because the nesting penalty needs a nesting level and javac erases it. Renders `docs/generated/endpoint-complexity.{html,json}` (ranked) and `endpoint-complexity-explained.html` (each method's source with a per-line annotation of what it cost and why, running total included). A report, not yet a fail-gate — no threshold is enforced |
...
| Structural lint (ast-grep) | Conventions CLAUDE.md states in prose but nothing checked | `.ast-grep/rules/` — currently `no-lombok` (error: Lombok was removed from `petclinic-backend`, so any `import lombok.…` is a reintroduction) and `requestbody-needs-valid` (warning). pre-commit scans the staged Java **and** the whole of `petclinic-backend/src` (~40ms, ungated) because a merge resolution never passes through a human's `git add`; `.githooks/pre-merge-commit` exists because `git merge` does not fire `pre-commit` at all. **CI re-runs the same scan.** Lombok is additionally gone from `pom.xml`, so a reintroduction cannot compile either |
