...
| `DeploymentDiagramTest` | Deployment diagram drift | Reads the mxGraph XML embedded in `petclinic-backend/docs/Deployment.drawio.png` (a real draw.io file: one artifact that both renders on GitHub and opens for editing in the desktop app) and compares its arrows against the browser suites' `petclinic-test/generated/*.genseq.puml`, generated from real OpenTelemetry traces (the `.java.*` diagrams in that folder are skipped: a MockMvc `@SpringBootTest` is deployed nowhere). Containers carry `traceParticipant="Browser|Backend|DB"`, arrows carry `traced="yes|no"`. Fails both ways — a traced arrow no trace shows, and a call the traces show that the picture does not draw — plus a third check that no arrow between two traced containers is left undeclared, so a new connection cannot opt out of the guardrail in silence |
...
| `DbSchemaExtractorTest` | DB schema snapshot | Boots Postgres, runs Flyway, dumps the schema to `petclinic-backend/docs/generated/DB.sql`; fails on drift |
...
| CODEOWNERS two-tier review | Sensitive files, two layers deep | `@victorrentea/tech-leads` review required for ordinary design-surface files: `openapi.yaml`, `petclinic-backend/docs/generated/DB.sql`, `db/migration/`, `docs/generated/DomainModel.puml`, `docs/packages.puml`, `docs/c4model.c1+c2.dsl`, `docs/c4model.c3.dsl`. `@victorrentea/elders` is reserved for the guardrail system itself: `.github/CODEOWNERS`, `.claude/settings.json`, `.github/workflows/*`, `.github/githooks/*`, the `guardrail/**` test sources, and the dependency surface those guardrails police (`**/pom.xml`, `**/package.json`, `petclinic-backend/pom-libs.txt`) |
