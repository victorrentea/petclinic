...
- [x] 1.1 Add a versioned Flyway migration in `petclinic-backend/src/main/resources/db/migration/`
...
- [x] 1.2 Add `@BatchSize` to `Owner.pets`, and verify a test that loads a page of owners with pets
...
- [x] 2.1 Create `PageDto<T>` (`content`, `totalElements`, `totalPages`, `number`, `size`) under
  `rest/dto/`, and verify it serializes to the documented JSON shape via a unit/mapper test
- [x] 2.2 Add a repository query on `OwnerRepository` that accepts `Pageable` and the `lastName`
...
- [x] 2.3 Update `OwnerRestController#listOwners` to accept `Pageable pageable` (via
...
- [x] 2.4 Add a sort-key allowlist check (`name`, `city`) in the controller, expanding `name` into
...
- [x] 2.5 Add an explicit `size > 20` guard returning 400, and verify with a test that `size=21`
  (and other over-cap values) is rejected while `size=20` succeeds
- [x] 2.6 Apply an `id ASC` tiebreaker and `NULLS LAST` (both directions) to every resolved `Sort`,
...
- [x] 2.7 Regenerate `openapi.yaml` via `OpenApiExtractorTest` (do not hand-edit), and verify the
  generated spec documents `page`/`size`/`sort` parameters and the new response envelope

## 3. Frontend: Owners grid

- [x] 3.1 Update the (currently orphaned) `OwnerPage` type and `OwnerService.getOwners` to call the
...
- [x] 3.2 Replace the Owners grid's plain `<table>` with `MatTable` + `MatSort` + `MatPaginator`
...
- [x] 3.3 Fix the pre-existing invalid markup for the `Pets` cell (`<tr>` nested inside a `<td>`),
  and verify the rendered table has exactly one `<tr>` per owner row
- [x] 3.4 Render the `Name` column as `"LastName, FirstName"` and wire its sort to `sort=name`, and
  verify with a component test that clicking the header issues a request with `sort=name`
- [x] 3.5 Wire the paginator to offer page sizes 5, 10, and 20, and verify selecting a size issues a
  new request with that `size` and `page=0`
- [x] 3.6 Make last-name search reset to page 0 while keeping the current sort, and verify with a
  component test that submitting a search resets `page` to 0 without changing `sort`

## 4. Tests and fixtures

- [x] 4.1 Update `OwnerTest`, `AddVisitSequenceTest`, `BasicAuthenticationConfigTest`, and
  `OwnerSearchThroughLatencyProxyTest` for the new response envelope, and verify they pass
- [x] 4.2 Update cucumber `OwnerSteps` and the Playwright E2E glue for the new envelope and grid
  markup, and verify the affected scenarios pass
- [x] 4.3 Rewrite the `every owner in the clinic is listed` E2E scenario (28 seeded owners no longer
...
- [x] 4.4 Update the `start-apps.ts` healthcheck for the new `GET /api/owners` response shape, and
  verify `./start-*.sh` scripts still report `✅ started` for all three apps

## 5. Full verification

- [x] 5.1 Run the full backend test suite (`mvn test` in `petclinic-backend/`) and verify all tests
  pass
- [x] 5.2 Run the full frontend test suite (`ng test` in `petclinic-frontend/`) and verify all tests
  pass
- [x] 5.3 Run the Playwright E2E suite in `petclinic-test/` against the running apps and verify all
  scenarios pass
