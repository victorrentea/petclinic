# Frontend Developer Agent Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Claude Code skill that auto-detects Angular/frontend work and delegates it to an isolated subagent with deep PetClinic Angular 16 conventions baked in — demonstrating context isolation equivalent to Copilot's custom agents.

**Architecture:** A single `SKILL.md` file registers as a Claude Code skill. When triggered (automatically or via `/frontend-dev`), it instructs the orchestrator to spawn a `general-purpose` Agent with a focused system prompt encoding all Angular patterns found in this codebase. The subagent operates only on `petclinic-frontend/` and returns a result to the orchestrator.

**Tech Stack:** Claude Code skill system (SKILL.md), Markdown, `Agent` tool (`general-purpose` subagent type)
...
| Action | Path | Purpose |
|--------|------|---------|
| Create | `.claude/skills/frontend-dev/SKILL.md` | Skill definition + subagent prompt |
...
**Files:**
- Create: `.claude/skills/frontend-dev/SKILL.md`

- [ ] **Step 1: Create the skill file with frontmatter and trigger description**

Create `.claude/skills/frontend-dev/SKILL.md` with this content:
...
description: >
  ...
  Auto-trigger when: user asks for frontend, Angular, UI, component, template, CSS work,
  or references files under petclinic-frontend/. Spawns an isolated subagent with full
  Angular 16 + Bootstrap 3 conventions for this project.
---
...
```bash
cat .claude/skills/frontend-dev/SKILL.md | head -20
```
Expected: frontmatter block with `name: frontend-dev` and description.

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/frontend-dev/SKILL.md
git commit -m "feat(agent): add frontend-dev skill with isolated Angular subagent"
```
...
In a Claude Code session in this project, type:
```
/frontend-dev Add a vaccination record list component for pets
```
Expected: Claude announces it is using the frontend-dev skill, spawns an Agent subagent, and the subagent returns Angular code following all conventions above (feature module structure, Bootstrap 3 classes, `catchError` pattern, template-driven form).

- [ ] **Step 2: Verify auto-detection**

Without using the slash command, type:
```
Add a new Angular component to display a pet's weight history
```
Expected: Claude detects this is frontend work, activates the skill automatically, and delegates to the subagent.

- [ ] **Step 3: Verify context isolation**

Ask the subagent (inside its output) something that only the parent context would know, e.g. reference a decision made earlier in the conversation. The subagent should not know about it — confirming isolation.
