...
public class OwnerSteps {
    ...
    @Then("the response page lists {int} owners")
    public void theResponsePageListsOwners(int expected) {
        assertThat(http.getLastResponse().jsonPath().getList("content").size()).isEqualTo(expected);
        assertThat(http.getLastResponse().jsonPath().getLong("totalElements")).isEqualTo(expected);
    }
