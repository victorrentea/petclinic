...
public class OwnerSteps {
    ...
    @Then("the response page holds {int} owner(s)")
    public void theResponsePageHoldsOwners(int expected) {
        assertThat(http.getLastResponse().jsonPath().getList("content").size()).isEqualTo(expected);
    }

    @Then("every owner in the response page has {string} equal to {string}")
    public void everyOwnerInTheResponsePageHasFieldEqualTo(String field, String value) {
