 ...
 * calling application stamps on every request — never as a tool argument (the LLM can neither see nor
 * set HTTP headers). {@code get_owner_profile} takes NO arguments, so the profile returned is
 * determined purely by the header: owner 1 (sub=1) gets Kevin McCallister, owner 2 (sub=2) gets Harry
 * Potter, and neither can reach the other's data. {@code X-API-Key} authenticates the calling
 * application (service account); the Bearer is the per-call on-behalf-of user assertion.
 */
...
class McpStreamableIdentityTest {
    ...
    void owner1_bearer_header_yields_owner1_profile_over_the_wire() {
        String profile = callGetOwnerProfileAs(jwtForOwner(1));
    ...
    void owner2_bearer_header_yields_owner2_profile_over_the_wire() {
        String profile = callGetOwnerProfileAs(jwtForOwner(2));
     ...
     * Bearer JWT (on-behalf-of, OUT-OF-BAND), then calls the no-argument {@code get_owner_profile}.
     */
    private String callGetOwnerProfileAs(String jwt) {
