...
class VisitDateRangeTest {
    private static final int MILTON = 3; // seeded: born 2020-09-07, owner 3
    ...
    void updateVisit_refusesADateOutOfRange(String date) throws Exception {
        int miltonsVisit = visitRepository.findByPetId(MILTON).get(0).getId();

        mockMvc.perform(put("/api/visits/{visitId}", miltonsVisit).contentType(MediaType.APPLICATION_JSON)
                .content("""
                        {"date": "%s", "description": "check-up"}""".formatted(date)))
                .andExpect(status().isBadRequest());
    }
