-- AUTO-GENERATED by applying all Flyway incremental migrations (db/migration/V*.sql) on an empty database.
...
CREATE TABLE public.owners (
    id integer NOT NULL,
    first_name text COLLATE pg_catalog.unicode,
    last_name text COLLATE pg_catalog.unicode,
    address text,
    city text COLLATE pg_catalog.unicode,
    telephone text
);
...
CREATE INDEX idx_owners_city ON public.owners USING btree (city, id);

CREATE INDEX idx_owners_last_name_prefix ON public.owners USING btree (last_name text_pattern_ops);

CREATE INDEX idx_owners_name ON public.owners USING btree (last_name, first_name, id);
