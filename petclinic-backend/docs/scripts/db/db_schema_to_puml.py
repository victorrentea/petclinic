...
The diagram is a plain snapshot of the *current* schema — no diff highlighting.
Comparing two snapshots (e.g. current vs the previously committed diagram at
review time) is the job of the separate puml-diff tool, not this generator, so
what lands in git is always a faithful picture of present reality.
...
def render_puml(cur: Schema) -> str:
    """Render the schema as a plain ER diagram (a snapshot of current reality)."""
    ...
    out = [
        ...
        "footer db/migration/*.sql -> DB -> dump to DB.sql -> converted to DB.puml",
        "hide circle",
