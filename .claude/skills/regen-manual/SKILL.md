---
name: regen-manual
description: Regenerates `user-manual/manual.md` by crawling the running PetClinic UI in a browser. Diff-based against the prior manual. Expensive (many browser steps + screenshots) — explicit `/regen-manual` invocation only.
...
2. A one-paragraph intro: what the app is for, plus the "do not edit by hand — regenerated by the `regen-manual` skill" line.
