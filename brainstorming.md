...
| D13 | **Sortare: coloană logică, expandată pe backend** | Frontendul trimite UN SINGUR criteriu = numele coloanei din grid (`name` / `city`) + direcție, ex. `sort=name,asc`. Backend-ul mapează coloana → câmpuri: `name`→`lastName`,`firstName`; `city`→`city`. Coloane necunoscute (inclusiv câmpuri brute ca `lastName`) respinse. **NU** se face bind direct pe `Pageable.sort`; controllerul citește params expliciți și construiește `PageRequest`. Un criteriu pe rețea; API-ul expune nume de coloane UI, nu câmpuri de entitate. |
...
| D16 | **Stilizare (frontend-ux)** | Header bg/border + bold ca tabelele Bootstrap surori; zebra pe rânduri impare; borders per-celulă; **`table-layout: fixed` + lățimi explicite per coloană** (obligatoriu la tabel sortabil, altfel coloanele sar la toggle sort); **celule cu `text-overflow: ellipsis`** (`overflow:hidden`+`white-space:nowrap`) ca valorile lungi să se trunchieze, nu să lățească coloana — lățimile alese din datele reale din DB la implementare; reuse `.btn-default` pt "Add Owner"; `white-space: nowrap` pe butoane/label-uri. Target CSS: `.mat-mdc-header-cell`, `.mat-mdc-cell`, `tr.mat-mdc-row:nth-child(odd)`, `.mat-column-<name>`. |
...
**Neafectat**: MCP (`PetClinicMcp`) folosește `OwnerRepository` direct; `petclinic-chatbot` citește owner-ii prin tool-ul MCP `get_owner_profile` și doar `/api/owners/{id}` (single, neschimbat) — **nu** lista. (Verificat cu subagent.)
...
- Lățimile per coloană (D16): **rezolvat** — le aleg la implementare din datele reale din DB, cu `text-overflow: ellipsis` pe celule (valorile lungi se trunchiază, nu lățesc coloana).
