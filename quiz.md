...
## Techniques to improve central agent context/skills:
...
- Paste all your coding conventions in the CLAUDE.md to make sure agent follows exactly your coding standards
...
## What can a skill do (in Claude Code)?
- Contain scripts (.sh, .py...) that automate deterministic sub-tasks ✅
- Contain nested skills❌
- Request to run in a separate context ± a particular model ✅ 
- Change agent permission❌
- Restrict tools that can be used in a new context ✅
...
## Best way to review the .md artifacts of the OpenSpec SDD framework?
...
## What change likely requires SDD, not just plan mode or direct prompting?
...
## Benefit of Gherkin tests for AI-built features?
...
## Example roles that can be assigned to separate agents
...
## What can MCP do that a plain REST API cannot?
- Return output in markdown format
- Bidirectional protocol between agent and MCP server (over HTTP+SSE) 
...
## AI yells "boss, I found the bug!" after just reading the code. What next?
...
## Agent permissions — recommended approach
...
## What is the Lethal Trifecta? (Simon Willison)
...
## Software supply-chain attack vectors against AI agents
...
## DETERMINISTIC guardrails against AI Slop:
...
## How to ENFORCE guardrails against AI Slop?
...
## Architecture surveillance to prevent design drift:
